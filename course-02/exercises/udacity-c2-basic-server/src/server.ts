...
(async () => {
  ...
  app.get( "/cars/:id", ( req: Request, res: Response ) => {
    ...
    if (car && car.length > 0) {
      res.status(200).send(cars.filter((car) => car.id == id));
    } else {
  ...
  app.post( "/cars", ( req: Request, res: Response ) => {
    ...
    // check if params are all there
    if ( !id || !type || !model || !cost ) {
      // return error if not
      return res.status(400).send("id, type, model, and cost required");
    }
    // instantiate new car from body
